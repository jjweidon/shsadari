...
const generatePastelColor = () => {
  ...
  const r = Math.floor(Math.random() * 120 + 130).toString(16).padStart(2, '0');
  const g = Math.floor(Math.random() * 120 + 130).toString(16).padStart(2, '0');
  const b = Math.floor(Math.random() * 120 + 130).toString(16).padStart(2, '0');
  return `#${r}${g}${b}`;
};

// 각 참가자에게 캐릭터 할당
const assignCharacters = (participants: string[]) => {
  const shuffledCharacters = [...CUTE_CHARACTERS].sort(() => Math.random() - 0.5);
...
const createTeams = (totalParticipants: number, membersPerTeam: number) => {
  ...
  if (remainder > 0 && remainder < numTeams) {
    ...
    for (let i = numTeams - 1; i >= numTeams - (membersPerTeam * numTeams - limitedParticipants); i--) {
      teamSizes[i] -= 1;
    }
  }
...
const calculateLadderResults = (
  grid: boolean[][],
  participantsWithCharacters: { name: string; character: string; color: string }[],
  teamSizes: number[]
) => {
  const verticalLines = participantsWithCharacters.length;
  const results: { start: number; end: number; path: { x: number; y: number }[] }[] = [];
  ...
  // 팀별 결과 구성
  const teamResults: Record<string, { teamName: string; members: { name: string; character: string; color: string }[] }> = {};
  ...
  participantResultsWithTeam
    .sort((a, b) => {
      const aPos = results.find(r => r.start === participantsWithCharacters.indexOf(a.participant))?.end || 0;
      const bPos = results.find(r => r.start === participantsWithCharacters.indexOf(b.participant))?.end || 0;
      return aPos - bPos;
    })
    .forEach(result => {
...
export default function Home() {
  ...
  // 참가자 목록 상태
  const [participantsWithCharacters, setParticipantsWithCharacters] = useState<{ name: string; character: string; color: string }[]>([]);
  ...
  const [results, setResults] = useState<{
    ...
    teamResults: Record<string, { teamName: string; members: { name: string; character: string; color: string }[] }>;
  } | null>(null);
  ...
  const [ladderDimensions, setLadderDimensions] = useState({ width: 0, height: 0 });
  ...
  const [previouslyAnimatedPlayers, setPreviouslyAnimatedPlayers] = useState<number[]>([]);
  ...
  const initializeLadder = () => {
    ...
    const initialPositions = Array(participantsWithCharacters.length).fill(null)
      .map((_, i) => ({ x: i, y: 0 }));
  ...
  // 단일 플레이어 애니메이션
  const animatePlayer = (playerIndex: number, results: { participant: { name: string; character: string; color: string }; team: string; path: { x: number; y: number }[] }[]) => {
    ...
    const interval = setInterval(() => {
      if (step < path.length - 1) { // path.length - 1까지만 진행 (다음 위치로 이동)
        // 현재 플레이어의 위치만 업데이트
        ...
        if (nextPos && currentPos && 
            nextPos.x !== undefined && nextPos.y !== undefined &&
            currentPos.x !== undefined && currentPos.y !== undefined) {
        ...
        setPreviouslyAnimatedPlayers(prev => [...prev, playerIndex]);
  ...
  // 모든 플레이어 함께 애니메이션
  const animateAllPlayers = (results: { participant: { name: string; character: string; color: string }; team: string; path: { x: number; y: number }[] }[]) => {
    // 가장 긴 경로 길이 찾기
    ...
    let step = 0;
    const interval = setInterval(() => {
      if (step < maxPathLength - 1) { // maxPathLength - 1까지만 진행 (다음 위치로 이동)
  ...
  const copyResults = () => {
    ...
    const text = Object.values(results.teamResults)
      .map(team => `${team.teamName}: ${team.members.map(m => `${m.character} ${m.name}`).join(', ')}`)
      .join('\n');
  ...
  const handleAddParticipant = () => {
    ...
    if (name && name.trim() !== '') {
      const newParticipant = {
        name: name.trim(),
        character: CUTE_CHARACTERS[Math.floor(Math.random() * CUTE_CHARACTERS.length)],
        color: generatePastelColor()
      };
  ...
  useEffect(() => {
    ...
    if (results && !isPlaying && animationComplete) {
      ...
      const resultParticipants = results.participantResults.map(r => 
        `${r.participant.name}-${r.participant.character}`
      ).join(',');
      
      const currentParticipants = participantsWithCharacters.map(p => 
        `${p.name}-${p.character}`
      ).join(',');
  ...
  const calculateLadderVisuals = () => {
    ...
    const verticalLinesElements = Array.from({ length: verticalLines }, (_, i) => {
    ...
    const nameElements = participantsWithCharacters.length <= 50 ? 
    ...
    const playerElements = participantsWithCharacters.map((participant, i) => {
      ...
      const isActive = moveAllAtOnce || (i === currentPlayerIndex) || previouslyAnimatedPlayers.includes(i);
    ...
    // 결과 계산 및 팀 배정 결과
    const teamAssignments = results ? results.participantResults.map(result => ({
    ...
    const sortedTeamAssignments = [...teamAssignments].sort((a, b) => {
      const aIndex = participantsWithCharacters.findIndex(p => p.name === a.position);
      const bIndex = participantsWithCharacters.findIndex(p => p.name === b.position);
    ...
    const teamElements = participantsWithCharacters.map((participant, i) => {
      ...
      // 해당 위치의 팀 찾기
      const team = sortedTeamAssignments[i]?.team || `팀 ${Math.floor(i / 3) + 1}`;
    ...
    <div className="min-h-screen p-6 bg-gray-50">
      <header className="text-center mb-8">
        <div className="flex items-center justify-center gap-2">
          ...
          <h1 className="text-3xl font-bold text-blue-600 mb-2">신한DS 금융SW 아카데미 사다리 게임</h1>
          <img src="/favicon.ico" alt="신한 로고" className="w-8 h-8" />
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <div className={`mb-6 ${isPlaying ? 'opacity-70' : ''}`}>
            <div className="flex flex-wrap items-center mb-4 gap-2">
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-500">URL에 data=이름1,이름2,... 형식으로 참가자를 추가할 수 있습니다.</p>
              </div>
              <button
            ...
            {participantsWithCharacters.length > 0 ? (
              <>
                <p className="mb-2 text-gray-700">총 참가자: {participantsWithCharacters.length}명</p>
                <div className={`grid ${
                  participantsWithCharacters.length > 50 ? 'grid-cols-3 sm:grid-cols-4 md:grid-cols-6' :
                  participantsWithCharacters.length > 20 ? 'grid-cols-2 sm:grid-cols-3 md:grid-cols-5' :
                  'grid-cols-2 sm:grid-cols-3 md:grid-cols-4'
                } gap-2 mb-6 overflow-y-auto max-h-80`}>
                  {participantsWithCharacters.map((participant, index) => (
                    <div key={index} className="flex items-center p-2 border rounded text-sm" 
                         style={{ backgroundColor: participant.color }}>
                      <span className="mr-1 text-lg">{participant.character}</span>
                      <span className="font-semibold text-gray-700 truncate">{participant.name}</span>
                    </div>
                  ))}
                </div>
              </>
              ...
              <div className="p-4 bg-yellow-50 text-yellow-700 rounded mb-4">
                참가자가 없습니다. URL 쿼리 파라미터(data=)를 사용하여 참가자를 추가하세요.
              </div>
            )}
          </div>
            ...
            <>
              <div className="mb-6">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-gray-700 font-bold mb-2">팀당 인원 수</label>
                    <select
                  ...
                  <div>
                    <label className="block text-gray-700 font-bold mb-2">사다리 표시</label>
                    <div className="flex items-center">
                      ...
                      />
                      <label htmlFor="showLadder" className="text-gray-500">사다리 선 보이기</label>
                    </div>
                  </div>
                  
                  <div>
                    <label className="block text-gray-700 font-bold mb-2">이동 방식</label>
                    <div className="flex items-center">
                      <input
                        type="checkbox"
                        id="moveAllAtOnce"
                        ...
                        className="mr-2 h-4 w-4 text-blue-600 focus:ring-blue-300"
                      />
                      <label htmlFor="moveAllAtOnce" className="text-gray-500">모두 함께 이동</label>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {(isPlaying || animationComplete) && (
            <div ref={ladderRef}>
              ...
              >
                {ladderDimensions.width > 0 ? calculateLadderVisuals() : (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Object.values(results.teamResults).map((team, index) => (
                      <div key={index} className="border rounded-lg p-4 bg-gray-50">
                        <h3 className="font-bold text-lg text-gray-800 mb-2">{team.teamName}</h3>
                        <div className="flex flex-wrap gap-2">
                          {team.members.map((member, i) => (
                            <div key={i} className="flex items-center px-3 py-1.5 rounded-full shadow-sm"
      ...
      <footer className="text-center mt-12 text-gray-500 text-sm">
        © <a href="https://github.com/jjweidon" target="_blank" rel="noopener noreferrer" className="hover:text-blue-600 transition-colors">jjweidon</a>
      </footer>
    </div>
  );
}
